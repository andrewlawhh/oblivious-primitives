...
template <typename T>
inline T extract(__m256i vec, int i);

template<>
inline int extract<int>(__m256i vec, int i) {
    return _mm256_extract_epi32_var_indx(vec, i);
}
template<>
inline float extract<float> (__m256i vec, int i) {
    return _mm256_extract_ps_var_indx((__m256)vec, i);
}

inline __m256i gather (float const * base, __m256i index, const int scale) {
    return (__m256i) _mm256_i32gather_ps(base, index, scale);
}
inline __m256i gather (int const * base, __m256i index, const int scale) {
...
inline T ObliviousArrayAccessSimd(const T *arr, size_t i, size_t n) {
    ...
    int vector_offset = (i - step_size * m) / elem_per_cache_line;
    ...
    for (j = 0; j + step_size <= n; j += step_size) {
      scanned = gather(&arr[j], indices, 4);
      maybe_retval = extract<T>(scanned, vector_offset);
...
// sizeof(T) == 32 bytes
template <typename T>
inline T o_assign_256(bool cond, T t_val, T f_val, T *out) {
    __m256i t_val_vector = _mm256_loadu_si256((__m256i*) &t_val);
    __m256i f_val_vector = _mm256_loadu_si256((__m256i*) &f_val);
    __m256i result_vector = _mm256_blend_epi32(f_val_vector, t_val_vector, ((int)cond) * 0xff);
    _mm256_storeu_si256((__m256i*) out, result_vector);
}

// sizeof(T) == 16 bytes
template <typename T>
inline T o_assign_128(bool cond, T t_val, T f_val, T *out) {
    T result;
    __m128i t_val_vector = _mm_loadu_si128((__m128i*) &t_val);
    __m128i f_val_vector = _mm_loadu_si128((__m128i*) &f_val);
    __m128i result_vector = _mm_blend_epi32(f_val_vector, t_val_vector, ((int)cond) * 0xf);
